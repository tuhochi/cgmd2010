...
public class Level {
	...
	public Level()
	{
		...
		mHouses.add(house);
		house = new HouseInfo();
		house.setPosition(new Vector3(-mBlockSize,-32.5f,0));
		house.setHouseSize(2);
		mHouses.add(house);
		house = new HouseInfo();
		house.setPosition(new Vector3(0.0f,-35.0f,mBlockSize));
		house.setHouseSize(3);
		mHouses.add(house);
		house = new HouseInfo();
		house.setPosition(new Vector3(0.0f,-37.5f,-mBlockSize));
	...
	public void update(float elapsedSeconds)
	{
		...
		for (HouseInfo house : mHouses) {
			if(house.getPosition().y + mPosition.y  + (house.getHouseSize() * mBlockSize / 2.0f)> 0)
			{
				...
				newHouse.setHouseSize(house.getHouseSize());
				newHouse.setPosition(new Vector3(house.getPosition().x, -mPosition.y - 50.0f, house.getPosition().z));
				add.add(newHouse);
			}
		}
